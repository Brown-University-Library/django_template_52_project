...
- Why: `run_tests.py` discovers tests under a top-level `tests/` package (see `run_tests.py` lines 51-64), but this project’s tests live under the Django app path `foo_app/tests/`.
...
- Add `.github/workflows/tests.yml` with the following high-level steps:
  ...
  - Provide environment and dirs expected by settings:
    ...
    - `mkdir -p ../logs ../DBs`
    - Optionally: `touch ../logs/foo_project.log` and `touch ../DBs/foo_project.sqlite` (not strictly required for SimpleTestCase, but safe)
...
- Logging: `LOG_PATH` points to `../logs/foo_project.log`. Ensure the parent directory exists in CI before Django loads logging.
